    ...
    return function(context){
      ...
      return new RSVP.Promise(function(resolve, reject){
        if (!context.addEventListener) {
          reject( {err: "Context has no `addEventListener`", context: context} );
          return;
        }

        context.addEventListener(resolveEvent, runloopify(resolve));
        context.addEventListener(rejectEvent, runloopify(reject));
