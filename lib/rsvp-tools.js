var RSVP = globals.RSVP || globals.Ember.RSVP,
    rsvpTools;
...
rsvpTools.evented = {
  ...
  //
  // var readAsDataUrl = rsvpTools.evented.contextPassed('readAsDataUrl', 'onload');
  ...
  contextPassed: function(target, resolveEvent, rejectEvent){
    return function(context){
      ...
      resolveEvent = resolveEvent || 'onsuccess';
      rejectEvent  = rejectEvent || 'onerror';

      return new RSVP.Promise(function(resolve, reject){
        context[resolveEvent] = runloopify(resolve);
        context[rejectEvent]  = runloopify(reject);
